---
title: 'Class Six: Multinomial and ordinal logistic regression'
author: "Xiaoyan Wang"
date: "Feb 22, 2018"
output:
...
-  Know how to predict and interpret the marginal effect
...
- Suppose our dependent nominal variable has J levels.
- Estimates a separate binary logistic regression model for each dummy variables, therefore the result is J-1 binary logistic regression models.
...
- Binary logit (2 nominal outcomes): we have 2-1 or one $\beta$ vector (i.e., parameters showing the effect of each predictor on the logit), including one intercept term $\beta0$
- Multinomial logit (J nominal outcomes): we have J-1 sets of parameters, or J-1 sets of $\beta$m (for m=1, 2, ... J-1). Each $\beta$m contains a non-zero intercept $\beta0$
- You can view binary logit as a special case of multinomial logit model in which J=2

##Example

- Breast cancer data was downlaoded from Surveillance, Epidemiology, and End Results Program (SEER), a premier data source for cancer statistics in the US. SEER collects incidence, prevalence and survival data.
...
Breast_SEER_Class6 <- read_delim("/Users/candice_wang/Box Sync/2018 Spring/TA/Class 6_my/Final/Breast_SEER_Class6.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
...
###Drop the cases who did not have first primary diagnosis of breast cancer and with missing values on any variables used.
...
- *Continous variable*
- A one-year increase in age increases the log odds of cases being diagnosed at stage I vs. stage IV by 0.021, holding other variables constant. 
- *Categorical variable*
- The log odds of being diagnosed at stage I vs. stage IV increases by 1.492 in cases without insurance compared to those with private insurance, holding all other variables constant. 
...
- Meaning: the log odds of being diagnosed in stage II vs. stage III is increase by 0.200 in those without insurance compared to those with private insurance, holding all other variables constant. 
...
#95% CI of Medicaid effect on having stage I vs.stage IV (inefficient method, one at a time)
...
- *Continous variable*
- The odds of being diagnosed with stage I breast cancer relative to stage IV are 1.02 times greater with each one-year increases in age, holding all other variables constant. 
- *Categorical variable*
- The odds of being diagnosed with stage I breast cancer relative to stage IV are 4.44 times greater for privately insured cases vs. uninsured cases, holding all other variables constant. 
...
- Marginal effects show the change in probability when the predictor or independent variable increases by one unit, holding all other independent variables constant at specific values. For continuous variables this represents the instantaneous change given that the units may be very small. For binary variables, the change is from 0 to 1.
...
- Due to the variable types inconsistence (e.g.,the mean value of Male variable may not be meaningful), however we can calculate by hand.
...
- Ordinal regression is used to predict the dependent variable with for ordered multiple categories and independent variables.
...
- Examples
- *Continous variable*
- The odds of being diagnosed at stage I is 0.973 times the odds of being diagnosed at *stages II to IV* with each one-year increase in age, holding other variables constant. 
- *OR*
- The odds of being diagnosed at *stage I and II* is 0.973 times the odds of being diagnosed at *stages III and IV* with each one-year increase in age, holding all other variables constant. 
- *Categorical variable*
- The odds of being diagnosed at *stage I* are 0.450 times the odds of being diagnosed at *stages II to IV* for patients with private insurance than those without insurance at diagnosis, holding all other variables constant.
- *OR*
- The odds of being diagnosed at *stage I and II* are 0.450 times the odds of being diagnosed at *stages III and IV* for patients with private insurance than those without insurance at diagnosis, holding other variables constant. 
...
- The probability of being diagnosed at stage IV for privately insured patients is lower than that for uninsured patients by 0.052;
- Every one-year increase in patients' age increases the probability of being diagnosed at stage I by 0.007.
